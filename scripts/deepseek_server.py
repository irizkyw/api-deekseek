...
from deepseek_api.cloudflare_bypasser_legacy import CloudflareBypasser
...
# Endpoint to chat with DeepSeek
@app.post("/chat")
async def chat(request: ChatRequest):
    try:
        token = os.getenv("DEEPSEEK_AUTH_TOKEN")
        if not token:
            raise HTTPException(status_code=500, detail="DEEPSEEK_AUTH_TOKEN not set in environment")
        ...
        async def event_generator():
            try:
                ...
                yield f"data: {json.dumps({'type': 'error', 'content': f'Authentication Error: {str(e)}'})}\n\n"
            except RateLimitError as e:
                yield f"data: {json.dumps({'type': 'error', 'content': f'Rate Limit Error: {str(e)}'})}\n\n"
            except NetworkError as e:
                yield f"data: {json.dumps({'type': 'error', 'content': f'Network Error: {str(e)}'})}\n\n"
...
if __name__ == "__main__":
    ...
    if (args.headless or DOCKER_MODE) and Display:
